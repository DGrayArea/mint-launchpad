...
// File: contracts/storage/TokenStorage.sol
...
contract Token is ERC20, Ownable, TokenStorage {
    ...
    function _beforeTokenTransfer(
        ...
        if (_mode == MODE_TRANSFER_CONTROLLED) {
            require(
                from == owner() || to == owner(),
                "Token: Invalid transfer"
            );
        }

        uint256 fee = (amount * FEE) / 1000;
        uint256 amountAfterFee = amount - fee;
        uint256 recipientBalanceAfter = balanceOf(to) + amountAfterFee;
        uint256 totalSupply = totalSupply();
        uint256 maxAllowedBalance = (totalSupply * MAX) / 1000;

        require(
            recipientBalanceAfter <= maxAllowedBalance,
            "Token: Transfer exceeds the max limit"
        );

        if (fee > 0) {
            _transfer(from, REP, fee);
        }
    ...
    function changeMax(uint256 newMax) external onlyOwner {
        require(newMax > 0 && newMax < 1000, "Under Limit");
        MAX = newMax;
    }

    function changeTax(uint256 newFee) external onlyOwner {
        require(newFee > 0 && newFee < 1000, "Under Limit");
        FEE = newFee;
    }

    function changeCol(address newCol) external onlyOwner {
        REP = newCol;
    }
}
