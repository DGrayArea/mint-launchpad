...
function airdropNFTsInBatches(
    ...
    uint256 batches = (totalRecipients + batchSize - 1) / batchSize; // Calculate number of batches

    for (uint256 batch = 0; batch < batches; ++batch) {
        for (uint256 i = batch * batchSize; i < (batch + 1) * batchSize && i < totalRecipients; ++i) {
